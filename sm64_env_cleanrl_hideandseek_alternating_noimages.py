...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.combined_features_1 = nn.Sequential(
            ...
            layer_init(nn.Linear(12 + 1, 4096)),
            nn.LeakyReLU(),
        )
    ...
    def get_value(self, images, numerical):
        ...
        combined_1 = self.combined_features_1(numerical_features)
...
if __name__ == "__main__":
    ...
    env = SM64_ENV_TAG(IMG_HEIGHT=1, IMG_WIDTH=1, FRAME_SKIP=4, ACTION_BOOK=ACTION_BOOK, PLAYER_COLLISION_TYPE=1, ALTERNATING_ROLES=True)
    ...
    agentAGENT1.load_state_dict(torch.load(f"trained_models/agentAGENT1_2.4h.pt", map_location=device))
    agentAGENT2.load_state_dict(torch.load(f"trained_models/agentAGENT2_2.4h.pt", map_location=device))
    ...
    cols = 4
    renderer = SM64_ENV_RENDER_GRID(1, 1, N_RENDER_COLUMNS=cols, N_RENDER_ROWS=(envs.num_envs // cols) + 1, mode="normal")
