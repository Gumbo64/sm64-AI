...
from env.sm64_env_curiosity_storage import CURIOSITY_STORAGE
...
def preprocess_space(space):
    ...
    space = gymnasium.spaces.Box(low=0, high=255, shape=new_img_shape, dtype=np.uint8)
    return space
##############################################
...
if __name__ == "__main__":
    ...
    img_shape = envs.single_observation_space.shape
    # print(img_shape)
    numerical_shape = (7, img_shape[2]) 
    ...
    curiosity_storage = CURIOSITY_STORAGE()
    ...
    for update in tqdm(range(1, num_updates + 1)):
        ...
        for step in tqdm(range(0, args.num_steps), leave=False):
            ...
            reward = curiosity_storage.get_rewards(infos)
            curiosity_storage.add_player_positions(infos)
            ...
            next_obs_numerical = torch.Tensor(curiosity_storage.get_numerical_obs(infos)).to(device)
