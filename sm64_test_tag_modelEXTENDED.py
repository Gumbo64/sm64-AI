...
agentHider.load_state_dict(torch.load(f"trained_models/agentHiderXL2.pt", map_location=device))

agentSeeker = Agent(envs).to(device)
agentSeeker.load_state_dict(torch.load(f"trained_models/agentSeekerXL2.pt", map_location=device))
