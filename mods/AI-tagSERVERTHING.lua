...
local function server_update()
    ...
    for i = 0, (MAX_PLAYERS-1) do
        if gNetworkPlayers[i].connected and gNetworkPlayers[i].globalIndex ~= 0 then
...
local function screen_transition(trans)
    ...
    if not s.seeking then
        for i=1,(MAX_PLAYERS-1) do
            if gNetworkPlayers[i].globalIndex ~= 0 and gNetworkPlayers[i].connected and gNetworkPlayers[i].currLevelNum == np.currLevelNum and
...
local function mario_update(m)
    ...
    if m.playerIndex == 0 then
		...
		elseif not gPlayerSyncTable[m.playerIndex].seeking and gGlobalSyncTable.displayTimer == 0 and gGlobalSyncTable.roundState == ROUND_STATE_ACTIVE then
			warp_to_level(LEVEL_BOB, 1, 0)
		elseif gPlayerSyncTable[m.playerIndex].seeking and gGlobalSyncTable.displayTimer == 5 and gGlobalSyncTable.roundState == ROUND_STATE_ACTIVE then
			warp_to_level(LEVEL_BOB, 1, 0)
		end
...
local function on_interact(m, obj, intee)
    if intee == INTERACT_PLAYER then
        ...
        if m ~= gMarioStates[0] then
            for i=0,(MAX_PLAYERS-1) do
                if gNetworkPlayers[i].globalIndex ~= 0 and gNetworkPlayers[i].connected and gNetworkPlayers[i].currAreaSyncValid then
