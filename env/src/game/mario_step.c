...
u32 mario_update_quicksand(struct MarioState *m, f32 sinkingSpeed) {
    ...
    if (m->action & ACT_FLAG_RIDING_SHELL || (gServerSettings.enableCheats && gCheats.godMode && /*m->playerIndex == 0*/ TRUE) || (!allow) || gDjuiInMainMenu) {
...
s32 perform_ground_step(struct MarioState *m) {
    ...
    if (gServerSettings.enableCheats && gCheats.superSpeed && /*m->playerIndex == 0*/ TRUE && m->action != ACT_BUBBLED) {
