...
class SM64_ENV_MIXED(SM64_ENV):
    def __init__(self, N_CLOSEST_PLAYERS=5, N_CLOSEST_NODES=10, N_PREVIOUS_POSITIONS=20,
        ...
        self.pos_scaler = 8000
        self.vel_scaler = 50

        super(SM64_ENV_MIXED,self).__init__(N_CLOSEST_PLAYERS=N_CLOSEST_PLAYERS, N_CLOSEST_NODES=N_CLOSEST_NODES, N_PREVIOUS_POSITIONS=N_PREVIOUS_POSITIONS, **kwargs)

    def init_with_max_players(self):
        super(SM64_ENV_MIXED,self).init_with_max_players()
        self.prev_positions = np.zeros((self.MAX_PLAYERS, self.N_PREVIOUS_POSITIONS, 3)) - self.pos_scaler

    def reset(self,seed=None,options=None):
        obss, infos = super(SM64_ENV_MIXED,self).reset(seed=seed,options=options)
        self.prev_positions = np.zeros((self.MAX_PLAYERS, self.prev_positions.shape[1], 3)) - self.pos_scaler
        return obss, infos

    def step(self,actions):
        obss, rews, terminations, truncations, infos = super(SM64_ENV_MIXED,self).step(actions)
