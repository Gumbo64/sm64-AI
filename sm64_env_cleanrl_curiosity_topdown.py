...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.network = nn.Sequential(
            # No frame stack so just 1 as the first number
            ...
            # 6272 calculated from torch_layer_size_test.py, given 4 channels and 128x72 input
            layer_init(nn.Linear(6272, 4096)),
            nn.LeakyReLU(),
...
if __name__ == "__main__":
    ...
    run_name = f"CURIOS_TOPDOWN_LSTMPPO_{int(time.time())}__LENGTH_{args.num_steps}_ACTIONS_{env.N_ACTIONS}_{env.IMG_WIDTH}x{env.IMG_HEIGHT}_PLAYERS_{env.MAX_PLAYERS}"
    ...
    # agent.load_state_dict(torch.load(f"trained_models/agentCuriosityTopdown_14.8+19.6h.pt", map_location=device))
